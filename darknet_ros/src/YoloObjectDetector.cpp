...
void *YoloObjectDetector::detectInThread()
{
  ...
  draw_detections_v3(display, dets, nboxes, demoThresh_, demoNames_, demoAlphabet_,
                  demoClasses_, 1);
...
void *YoloObjectDetector::displayInThread(void *ptr)
{
  ...
  int c = wait_key_cv(waitKeyDelay_);
  if (c == 27)
  {
    demoDone_ = 1;
    return 0;
...
void YoloObjectDetector::yolo()
{
  ...
  output_image_ = cv::Mat(buff_[0].w, buff_[0].h, CV_MAKETYPE(CV_8U, buff_[0].c));
  ...
  while (!demoDone_ && ros::ok())
  {
    ...
    if (!demoPrefix_)
    {
      ...
      {
        generate_image(buff_[(buffIndex_ + 1) % 3], output_image_);
      }
      publishInThread();
    }
    else
...
void *YoloObjectDetector::publishInThread()
{
  ...
  if (!publishDetectionImage(output_image_))
  {
